export default class Base {
  ...
  initialize() {
    ...
    //ua
    const uaOS = window.navigator.userAgent;
    const ua = uaOS.toLowerCase();
    const bodyElem = document.body;

    //os
    const Windows = uaOS.match(/Windows NT (\d+\.\d+)/),
    ...
    //browser
    const BrowserSafari  = ua.indexOf("safari") > -1 && ua.indexOf("chrome") == -1,
    ...
    if(ua.indexOf("msie") != -1) {
      const noScriptText = `<div class="is-prompt" data-elements="add-js">
    ...
    if ((/Android/.test(uaOS) && /Linux; U;/.test(uaOS) && !/Chrome/.test(uaOS)) ||
      ...
      const noAndroidText = `<div class="is-prompt" data-elements="add-js">
